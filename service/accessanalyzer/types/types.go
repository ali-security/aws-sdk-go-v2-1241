...
// Specifies the configuration for a policy preview analysis, including the type
// of analysis, the target resource, and the policy documents to evaluate.
type PolicyConfiguration struct {

	// The type of impact analysis job. Currently only SCP (Service Control Policy) is
	// supported.
	//
	// This member is required.
	JobType ImpactAnalysisJobType

	// A list of SCP policy documents to test. Each policy document is a JSON string
	// with a maximum length of 5,120 characters. The analysis evaluates how these
	// policies would affect access to resources.
	//
	// This member is required.
	PolicyDocumentsList []string

	// The identifier of the target resource for the policy analysis. This can be an
	// Amazon Web Services account ID (12-digit number), an organization root ID
	// (format: r-[0-9a-z]{4,32} ), or an organizational unit ID (format:
...
type PolicyPreviewAnalysisReport struct {
...
// Contains the configuration details for policy preview, including the scope,
// status, and timestamps.
type PolicyPreviewConfiguration struct {
	...
	// The status of the policy preview configuration. A value of ACTIVE indicates the
	// configuration is enabled and CloudTrail authorization events are being
	// collected.
	//
	// This member is required.
...
type PolicyPreviewJobDetails struct {
	...
	//
	// This member is required.
	JobStatus ImpactAnalysisJobStatus
	...
	// The time at which the job completed. This field is populated only when the job
	// reaches a terminal state (COMPLETED, FAILED, or CANCELED).
	CompletedAt *time.Time

	// Detailed information about the error that caused the job to fail. This field is
	// populated only when the job status is FAILED.
	JobError *JobError

	// The time at which the job execution started. This field is not populated until
	// the job begins processing.
	StartedAt *time.Time
...
type PolicyPreviewJobParameters struct {
